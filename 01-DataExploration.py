...
genome_scores_df = pd.read_csv('./data/ml-latest/genome-scores.csv')
print("genome_scores_df")
print(genome_scores_df.head())

genome_tags_df = pd.read_csv('./data/ml-latest/genome-tags.csv')
...
data[['userId', 'rating', 'movieId', 'title']].sort_values('userId').to_csv('./data/merged.csv', index=False)
